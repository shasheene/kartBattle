...
glmReadMTL(GLMmodel* model, char* name)
{
  ...
  for (i = 0; i < nummaterials; i++) {
    ...
    model->materials[i].map_Kd_file = NULL;
  }
  ...
  //Suter 
      ...
      strcpy(filename, dir);
      strcat(filename, buf);
      model->materials[nummaterials].map_Kd_file = strdup(filename);
     free(dir);
     free(filename);
      fprintf(stderr,"added Kd file %s\n",model->materials[nummaterials].map_Kd_file);
      break;
    case 'K':
      switch(buf[1]) {
      case 'd':
